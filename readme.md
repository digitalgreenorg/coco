...
3.	/api/api_name/ : In order to communicate with the database using RESTful service, the url for the REST API needs to be specified in the configs.js file in the parameter ërest_api_urlí. Its not compulsory to use 

/api/ URL, but it is recommended. 

4.	/coco/reset_database_check/ : This URL can be changed in configs file. COCO sends a get request to this url with a parameter 

ëlastdownloadtimestampí. The server sends ì1î as a response if it wants the client to download the database again. 
...
user = models.OneToOneField(User) states = models.ManyToManyField(State)
...
bundle = super(BaseResource, self).full_hydrate(bundle) bundle.obj.user_modified_id = bundle.request.user.id return bundle
...
for	key, value in kwargs.items(): setattr(bundle.obj, key, value)

self.authorized_create_detail(self.get_object_list(bundle.request),

bundle)

bundle = self.full_hydrate(bundle) bundle.obj.user_created_id = bundle.request.user.id return self.save(bundle)

b.	Add a function dict_to_foreign_uri which generate a foreign uri for a given field in a dictionary. 

def	dict_to_foreign_uri(bundle, field_name, resource_name=None): field_dict = bundle.data.get(field_name)
if field_dict:
...
class StateLevelAuthorization(Authorization): def __init__(self, field):
...
# Is the requested object owned by the user? kwargs = {}

kwargs[self.state_field] = CocoUser.objects.get(user_id= bundle.request.user.id).get_state()

obj = object_list.filter(**kwargs).distinct() if obj:
...
timestamp = models.DateTimeField(default=datetime.datetime.utcnow) user = models.ForeignKey(User, null = True)

4

 

state = models.IntegerField(null = True) action = models.IntegerField()

entry_table = models.CharField(max_length=100) model_id = models.IntegerField(null = True)
...
post_save.connect(save_log, sender = Progress) pre_delete.connect(delete_log, sender = Progress)
...
class FullDownloadStats(models.Model): user = models.ForeignKey(User) start_time = models.DateTimeField() end_time = models.DateTimeField()
...
var person_configs = { 'entity_name': 'person',

'rest_api_url': '/coco/api/v1/person/', 'page_header': 'Person', 'list_table_header_template': 'person_table_template',

'list_table_row_template': 'person_list_item_template', 'sort_field': 'person_name',

'add_template_name': 'person_add_edit_template', 'edit_template_name': 'person_add_edit_template', 'foreign_entities': {
...
'unique_together_fields': ['person_name', 'father_name', 'village.id'], 'form_field_validation': {
...
entity_name	String	Name of the object in indexed DB. It is used for
		accessing this object.
		
rest_api_url	String	The REST url for this entity
		
dashboard_display	dictionary	Determine if list view and add view of this entity is
		required or not. By default everything in dictionary
		is true.
		
page_header	String	The name that needs to shown in headers (on
		dashboard as well as list view)
		
list_table_header_tem	HTML	The id of template in dashboard.html used as
plate	template	column headers in list page.
...
list_table_row_templ
HTML		The id of template in dashboard.html used to create
ate	template		rows of list table. This template is passed the model
			json.
				
sort_field	String		The name of the field (should be same as in json
			received from REST) on which the data should be
			sorted by default in list view.
				
add_template_name	HTML		The id of template in dashboard.html used to add
	template		data.
				
edit_template_name	HTML		The id of template in dashboard.html used to edit
	template		data.
			
foriegn_entities	dictionary	Configuration of the foreign elements used by this
			entity
				
unique_together_fiel	list	of	The name of the attributes whose combination
d	strings		should be unique. This thing is checked when user
			clicks on submit in a form.
...
enable_months	list	of	Index of  months for which add should be	
	numbers		enabled. (Starts from 1)	
				

foreign_entities dictionary is of form: foreign_entities : {

foreign_entity_name: { attribute_name_in _json:{
